<?php
    ...
    if(isset($_POST['login']))
    {
        ...
        $stmt = mysqli_prepare($con, "SELECT * FROM tbladmin WHERE staffId = ? AND password = ?");
        $hashedPassword = md5($password); // Still using MD5 for compatibility, but should be upgraded
        mysqli_stmt_bind_param($stmt, "ss", $staffId, $hashedPassword);
